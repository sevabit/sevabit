...
Sevabit is a privacy-focused cryptocurrency based on Monero/Sevabit. Sevabit currently offers an incentivised full node layer, over the coming months we will be looking to support a secondary p2p network (Sevabitnet) and a messenger that offers private communications based on the Signal protocol (Sevabit Messenger).
...
* Clone Sevabit and checkout the most recent release version:

    ```bash
    git clone https://github.com/sevabit-project/sevabit.git
    cd sevabit
    git checkout master
    ```
...
* To git clone, run:
    ...
    ```bash
    git clone --recursive https://github.com/sevabit-project/sevabit.git
    ```
...
* If you would like a specific [version/tag](https://github.com/sevabit-project/sevabit/tags), do a git checkout for that version. eg. 'v3.0.6'. If you don't care about the version and just want binaries from master, skip this step:
...
The project can be built from scratch by following instructions for Linux above(but use `gmake` instead of `make`). If you are running sevabit in a jail you need to add the flag: `allow.sysvipc=1` to your jail configuration, otherwise lmdb will throw the error message: `Failed to open lmdb environment: Function not implemented`.

We expect to add Sevabit into the ports tree in the near future, which will aid in managing installations using ports or packages.
...
Build sevabit: `env DEVELOPER_LOCAL_TOOLS=1 BOOST_ROOT=/usr/local gmake release-static`

Note: you may encounter the following error, when compiling the latest version of sevabit as a normal user:
...
* Docker
    ...
    docker build -t sevabit-daemon-image .
    
    # or build using a specific number of cores (reduce RAM requirement)
    docker build --build-arg NPROC=1 -t sevabit .
    
    # either run in foreground
    docker run -it -v /sevabit/chain:/root/.sevabit -v /sevabit/wallet:/wallet -p 22022:22022 sevabit
    
    # or in background
    docker run -it -d -v /sevabit/chain:/root/.sevabit -v /sevabit/wallet:/wallet -p 22022:22022 sevabit
    ```
...
[sevabitd.service](utils/systemd/sevabitd.service) to `/etc/systemd/system/` and
[sevabitd.conf](utils/conf/sevabitd.conf) to `/etc/`. The [example
service](utils/systemd/sevabitd.service) assumes that the user `sevabit` exists
and its home is the data directory specified in the [example
config](utils/conf/sevabitd.conf).

If you're on Mac, you may need to add the `--max-concurrency 1` option to
sevabit-wallet-cli, and possibly sevabitd, if you get crashes refreshing.
