# Anonymity Networks with Sevabit

Currently only Tor and I2P have been integrated into Sevabit. The usage of
...
is made or until `sevabitd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `sevabit-wallet-cli` and
`sevabit-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--proxy` which tells `sevabitd` the ip/port of a socks proxy provided by a
separate process. On most systems the configuration will look like:

> `--proxy tor,127.0.0.1:9050,10`
> `--proxy i2p,127.0.0.1:9000`

which tells `sevabitd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `sevabitd` the inbound address, network
type, and max connections:

> `--anonymous-inbound rveahdfho7wo4b2m.onion:28083,127.0.0.1:28083,25`
> `--anonymous-inbound cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000,127.0.0.1:30000`

which tells `sevabitd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `sevabitd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `sevabitd` localhost port 30000.
...
`sevabitd` RPC port - which is independent from the configuration for incoming
...
> HiddenServiceDir /var/lib/tor/data/sevabit
...
This will store key information in `/var/lib/tor/data/sevabit` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/sevabit/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Sevabit network should have
similar timestamps.
...
An ISP can passively monitor `sevabitd` connections from a node and observe when
...
only using I2P for broadcasting Sevabit transactions, the total aggregate of
I2P data would also leak information.

#### Mitigation

There is no current mitigation for the user right now. This attack is fairly
sophisticated, and likely requires support from the internet host of a Sevabit
user.
...
### Intermittent Sevabit Syncing

If a user only runs `sevabitd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `sevabitd` as often as possible to conceal when transactions are being sent.
