...
namespace super_nodes {
  // Service node decommissioning: as super nodes stay up they earn "credits" (measured in blocks)
  // towards a future outage.  A new super node starts out with INITIAL_CREDIT, and then builds up
  // CREDIT_PER_DAY for each day the super node remains active up to a maximum of
  // DECOMMISSION_MAX_CREDIT.
  //
  // If a super node stops sending uptime proofs, a quorum will consider whether the super node
  // has built up enough credits (at least MINIMUM): if so, instead of submitting a deregistration,
  // it instead submits a decommission.  This removes the super node from the list of active
  // super nodes both for rewards and for any active network duties.  If the super node comes
  // back online (i.e. starts sending the required performance proofs again) before the credits run
  // out then a quorum will reinstate the super node using a recommission transaction, which adds
  // the super node back to the bottom of the super node reward list, and resets its accumulated
  ...
  static_assert(CHECKPOINT_MAX_MISSABLE_VOTES < CHECKPOINT_MIN_QUORUMS_NODE_MUST_VOTE_IN_BEFORE_DEREGISTER_CHECK,
                "The maximum number of votes a super node can miss cannot be greater than the amount of checkpoint "
  ...
  // The lower swarm percentile that will be randomly filled with new super nodes
...
// return: UINT64_MAX if (num_contributions > the max number of contributions), otherwise the amount in sevabit atomic units
...
bool check_super_node_portions(uint8_t version, const std::vector<uint64_t>& portions);
